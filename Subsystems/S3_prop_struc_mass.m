function [m_prop,m_SC,cost,S3_constraints] = S3_prop_struc_mass(delta_v_escape,delta_v_arrival, departure_date, arrival_date, Isp)
    ...
    m_structure = 1000;
    m_payload = 1000;
    m_prop = (m_structure + m_payload)*(1-exp(-1*delta_v/(9.81*Isp)));
    ...
    %c1 = m_SC*(1-exp(-1*delta_v/(g*Isp)));     % m_prop >= m_SC*(1-^(-delta_v/g*Isp))
    ...
    c2 = m_minshield + D_total/(alpha) - beta*m_structure;  % beta*m_structure = m_minshield + alpha*(D_total)
