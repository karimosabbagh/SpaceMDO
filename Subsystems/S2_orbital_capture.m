function [delta_v_capture, V_SC_arrival, S2_constraints] = S2_orbital_capture(e, r_p2, V_SC_departure, departure_date, arrival_date)
...
addpath(subsytems);
 ...
 global G M_Mars M_Sun earth_orbital_data mars_orbital_data;
    ...
    V_SC_arrival = sqrt(V_SC_departure^2 + 2 * G * (M_Sun) * ...
        ((1 / R_Mars_arrival) - (1 / R_Earth_departure)));
    
    % hyperbolic escape velocity
    V_infinity_A = V_Mars_arrival -  V_SC_arrival;

    delta_v_capture = sqrt(V_infinity_A ^ 2 + 2 * G * (M_Mars) / (r_p2)) + ... 
        sqrt(G * (M_Mars ) * (1 + e) / (r_p2));
